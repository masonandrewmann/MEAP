...
#include <tables/m_power_curve.h>
...
public:
    ...
    void setFilterType(uint16_t f_t)
    {
        ...
        if (filter_type_ < 128)
        {
            filter_low_amt_ = m_power_curve_DATA[(127 - filter_type_) << 1];
            filter_high_amt_ = m_power_curve_DATA[filter_type_ << 1];
            filter_band_amt_ = 0;
        }
        else
        {
            filter_high_amt_ = m_power_curve_DATA[(255 - filter_type_) << 1];
            filter_band_amt_ = m_power_curve_DATA[(filter_type_ - 127) << 1];
            filter_low_amt_ = 0;
        }
    }

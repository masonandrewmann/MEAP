...
int updateAudio(){
  int asig = (aOscil.next() * gain)>>8;

  return MonoOutput::fromAlmostNBit(8, aDel.next(asig, deltime)); // mix some straight signal with the delayed signal
//return MonoOutput::from8Bit(asig); // mix some straight signal with the delayed signal
}

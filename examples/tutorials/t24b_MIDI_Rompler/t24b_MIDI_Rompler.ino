...
#define ROMPLER_POLYPHONY 12
mRompler<300000, ROMPLER_POLYPHONY, int16_t> rompler;
...
void setup()
{
  ...
  rompler.init(rompler_list, rompler_lengths);
  rompler.setADSR(1, 100, 255, 500);
  rompler.setProgram(0);
}
...
{
  int64_t out_sample = rompler.next();
...
void clockStep()
{
  ...
  if (clock_pulse_num % 24 == 0)
  { // quarter note
  }

  if (clock_pulse_num % 12 == 0)
  { // eighth note
  }

  if (clock_pulse_num % 6 == 0)
  { // sixteenth note
  }

  if (clock_pulse_num % 3 == 0)
  { // thirtysecond notex
  }

/*
  Generates two random sets of harmonics.

  Triggers regular notes. Each note begins by using first set of harmonics then smoothly interpolates to the other one by the end of the note
...
Oscil<SIN8192_NUM_CELLS, AUDIO_RATE> osc[NUM_OSC](SIN8192_DATA);
int gains1[NUM_OSC];
int gains2[NUM_OSC];
int actualGains[NUM_OSC];
int interpVal = 0;
...
void updateControl(){
  ...
  // interpVal = map(potVals[0]
  long currTime = millis();
  for(int i = 0; i < NUM_OSC; i++){
    // actualGains[i] = map(potVals[0], 0, 4095, gains1[i], gains2[i]);
    actualGains[i] = map(currTime, noteStart, noteEnd, gains1[i], gains2[i]);
  }

  if(noteTrig.ready()){
    generateHarmonics();
    noteTrig.start(noteDelay);
    env.start();
    noteStart = millis();
    noteEnd = noteStart + 3000;
  }
...
int updateAudio(){
  int sample = 0;
  for(int i = 0; i < NUM_OSC; i++){
    sample += osc[i].next() * actualGains[i];
  }
  return MonoOutput::fromAlmostNBit(27, sample * envGain);
}
...
void generateHarmonics(){
    for(int i = 0; i < NUM_OSC; i++){
    osc[i].setFreq(220 * (i+1));
    gains1[i] = irand(0, map(i, 0, 20, 150, 0));
    // gains1[i] = 255/i;
    gains2[i] = irand(0, map(i, 0, 20, 255, 50));
    actualGains[i] = 0;
  }
}

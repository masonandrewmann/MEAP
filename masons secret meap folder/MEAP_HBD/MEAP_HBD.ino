...
int transitionTable[8][8] = 
{
  {38, 25,  0, 13, 13,  0,  0, 13}, //from C 
  {66,  0,  0,  0,  0,  0, 33,  0}, //from D
  {50, 50,  0,  0,  0,  0,  0,  0}, //from E 
  {25,  0, 50,  0, 25,  0,  0,  0}, //from F 
  { 0,  0,  0,100,  0,  0,  0,  0}, //from G 
  { 0,  0,  0,100,  0,  0,  0,  0}, //from A 
  { 0,  0,  0,  0,  0, 50, 50,  0}, //from B
  { 0,  0,  0,  0,  0,100,  0,  0}  //from C^ 
};
int transitionProbs[8][8];
...
int noteVals[] = {48, 50, 52, 53, 55, 57, 58, 60};
int lenVals[] = {63, 188, 250, 500};
...
void setup(){
  ...
  //computer note probabilities from transition table
  for (int i = 0; i < 8; i++){
    for (int j = 0; j < 8; j++){
      if( j == 0){
        transitionProbs[i][j] = transitionTable[i][j];
      } else{
        transitionProbs[i][j] = transitionTable[i][j] + transitionProbs[i][j-1]; 
      }
      Serial.print(transitionProbs[i][j]);
      Serial.print("  ");
    }
    Serial.println(" ");
  }
  ...
  //computer length probabilities from transition table
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      if( j == 0){
        lenProbs[i][j] = lenTable[i][j];
      } else{
        lenProbs[i][j] = lenTable[i][j] + lenProbs[i][j-1]; 
      }
      Serial.print(lenProbs[i][j]);
      Serial.print("  ");
    }
    Serial.println(" ");
  }
...
//  readPots(); // reads potentiometers
  if (noteDelay.ready()){
    ...
    currNote = chooseNote(currNote);
    mySine.setFreq(mtof(noteVals[currNote]));
    
    currLen = chooseLen(currLen);
    noteDelay.start(lenVals[currLen]*2);
  }
  envelope.update();
  gain = envelope.next();
}
...
//  uint16_t myVal = mySine.next();
  return MonoOutput::from16Bit(mySine.next() * gain);
}
...
int chooseNote(int myNote){
  int randNum = rand(100);
  for (int j = 0; j < 8; j++){
    if (randNum < transitionProbs[myNote][j]){
      return j;
    }
  }
}

int chooseLen(int myLen){
  int randNum = rand(100);
  for (int j = 0; j < 4; j++){
    if (randNum < lenProbs[myLen][j]){
      return j;
    }
  }
}

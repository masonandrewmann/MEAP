...
MySample<love_song_NUM_CELLS, AUDIO_RATE> sample[NUM_SAMPLE_TRACKS];  //maybe change the MAX_SAMPLE_LENGTH? calc how it corresponds to seconds etc
...
MySample<love_song_NUM_CELLS, AUDIO_RATE> granular[NUM_GRANULAR];  //maybe change the MAX_SAMPLE_LENGTH? calc how it corresponds to seconds etc
...
int pulse_loop_step[PULSE_VOICES] = { 48, 48, 48, 48, 20, 4, 8, 36 };
...
void setup() {
  ...
  pulse_loop_step[0] = 144;
  pulse_loop_step[1] = 144;
  pulse_loop_step[2] = 144;
  pulse_loop_step[3] = 144;

  pulse_curr_step[0] = 0;
  pulse_curr_step[1] = 48;
  pulse_curr_step[2] = 96;
  pulse_curr_step[3] = 144;
}
...
void samplePulse(int voice_num, int midi_note_num, int pulse_sample_num) {
  ...
  pulse_env[voice_num].setAttackTime(4);
...
void chordTreeStep() {
  ...
  pulse_note[0] = note_bank[0];
  pulse_note[1] = note_bank[2];
  pulse_note[2] = note_bank[3];
  pulse_note[3] = note_bank[1];
}
...
void tempoTreeStep() {
  ...
  updateTempo(map(meap.pot_vals[0],0, 4095, 30, 400));

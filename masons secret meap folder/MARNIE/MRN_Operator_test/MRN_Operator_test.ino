...
mOperator<SIN2048_NUM_CELLS, AUDIO_RATE, CONTROL_RATE, int8_t> car_op(SIN2048_DATA);

mOperator<SIN2048_NUM_CELLS, AUDIO_RATE, CONTROL_RATE, int8_t> mod_op(SIN2048_DATA);

mOperator<SIN2048_NUM_CELLS, AUDIO_RATE, CONTROL_RATE, int8_t> aux_op(SIN2048_DATA);
void setup() {
  ...
  mod_op.setFreqRatio(2.0);
  aux_op.setFreqRatio(7.0);
}
...
void updateControl() {
  ...
  car_op.setAttackTime(map(meap.aux_mux_vals[6]>>4, 0, 255, 1, 3000));
  car_op.setReleaseTime(map(meap.aux_mux_vals[5]>>4, 0, 255, 1, 3000));

  mod_op.setAttackTime(map(meap.aux_mux_vals[4]>>4, 0, 255, 1, 3000));
  mod_op.setReleaseTime(map(meap.aux_mux_vals[3]>>4, 0, 255, 1, 3000));

  aux_op.setAttackTime(map(meap.aux_mux_vals[0]>>4, 0, 255, 1, 3000));
  aux_op.setReleaseTime(map(meap.aux_mux_vals[1]>>4, 0, 255, 1, 3000));

  mod_op.setGain(meap.pot_vals[1] >> 4);
  aux_op.setGain(meap.pot_vals[0] >> 4);

  car_op.update();
  mod_op.update();
  aux_op.update();

  vel = meap.aux_mux_vals[2]>>4;
}


AudioOutput_t updateAudio() {
  int32_t out_sample = car_op.next(mod_op.next() + aux_op.next());
...
void Meap::updateTouch(int number, bool pressed) {
  ...
  if (pressed) {
    car_op.noteOn(notes[number], vel);
    mod_op.noteOn(notes[number], vel);
    aux_op.noteOn(notes[number], vel);
  } else {
    car_op.noteOff();
    mod_op.noteOff();
    aux_op.noteOff();
      ...
      switch (i) {
        case 0:
          if (button_vals[i]) {  // button 0 down
            ...
            car_op.setLoopingOn();
            // car_op.noteOn(58, 127);
            // mod_op.noteOn(58, 127);
          } else {  // button 0 up
            Serial.println("Button 0 up");
            car_op.setLoopingOff();
            // car_op.noteOff();
            // mod_op.noteOff();
          }
          break;
        case 1:
          if (button_vals[i]) {  // button 1 down
            ...
            mod_op.setLoopingOn();
            // car_op.noteOn(60, 127);
            // mod_op.noteOn(60, 127);
          } else {  // button 1 up
            Serial.println("Button 1 up");
            mod_op.setLoopingOff();
            // car_op.noteOff();
            // mod_op.noteOff();
          }
          break;

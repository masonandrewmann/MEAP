...
enum my_interpolation {kINTERP_NONE, kINTERP_LINEAR};
...
*/
template <uint32_t NUM_TABLE_CELLS, unsigned int UPDATE_RATE, uint8_t INTERP=kINTERP_NONE>
class MySample
{
...
public:
	...
	*/
	MySample(const int8_t * TABLE_NAME):table(TABLE_NAME),endpos_fractional((uint32_t) NUM_TABLE_CELLS << SAMPLE_F_BITS) // so isPlaying() will work
	{
		setLoopingOff();
		//rangeWholeSample();
	}
	...
	*/
	MySample():endpos_fractional((uint32_t) NUM_TABLE_CELLS << SAMPLE_F_BITS)
	{
		setLoopingOff();
		//rangeWholeSample();
	}
	...
	*/
	inline
	void setStart(uint32_t startpos)
	{
	...
	*/
	...
	int8_t next() { // 4us
		...
		if(INTERP==kINTERP_LINEAR){
			...
			int8_t diff_fraction = (int8_t)(((((uint32_t) phase_fractional)>>8)*difference)>>8); // (unsigned int) phase_fractional keeps low word, then>> for only 8 bit precision
			out += diff_fraction;
		}else{
	...
	*/
	inline
	uint32_t phaseIncFromFreq(unsigned int frequency)
	{
...
static const uint8_t ADJUST_FOR_NUM_TABLE_CELLS = (NUM_TABLE_CELLS<2048) ? 8 : 0;
	...
	volatile uint32_t phase_fractional;
	volatile uint32_t phase_increment_fractional;
	const int8_t * table;
	bool looping;
	uint32_t startpos_fractional, endpos_fractional;
};

...
*/
template <uint64_t NUM_TABLE_CELLS, unsigned int UPDATE_RATE, uint8_t INTERP=kINTERP_NONE>
class MySample
{
...
public:
	...
	*/
	MySample(const int8_t * TABLE_NAME):table(TABLE_NAME),endpos_fractional((uint64_t) NUM_TABLE_CELLS << SAMPLE_F_BITS) // so isPlaying() will work
	{
		setLoopingOff();
		//rangeWholeSample();
	}
	...
	*/
	MySample():endpos_fractional((uint64_t) NUM_TABLE_CELLS << SAMPLE_F_BITS)
	{
		setLoopingOff();
		//rangeWholeSample();
	}
	...
	*/
	inline
	void setStart(uint64_t startpos)
	{
	...
	*/
	...
	int8_t next() { // 4us
		...
		if(INTERP==kINTERP_LINEAR){
			...
			int8_t diff_fraction = (int8_t)(((((uint64_t) phase_fractional)>>8)*difference)>>8); // (unsigned int) phase_fractional keeps low word, then>> for only 8 bit precision
			out += diff_fraction;
		}else{
	...
	*/
	inline
	uint64_t phaseIncFromFreq(unsigned int frequency)
	{
	 ...
	 */
	inline
	void setPhaseInc(uint64_t phaseinc_fractional)
	{

...
#define NUM_SPARKLES 6
mSampler16<300000, AUDIO_RATE> sparkle[6];
uint8_t sparkle_sample_num = 0;
...
void setup() {
  ...
  // sparkly 
  sparkle_mode = kINACTIVE;
  sparkle[0].setTable(samples_list[4], samples_lengths[4]);
  sparkle[1].setTable(samples_list[5], samples_lengths[5]);
  sparkle[2].setTable(samples_list[4], samples_lengths[4]);
  sparkle[3].setTable(samples_list[6], samples_lengths[6]);
  sparkle[4].setTable(samples_list[4], samples_lengths[4]);
  sparkle[5].setTable(samples_list[1], samples_lengths[1]);
}
...
AudioOutput_t updateAudio() {
  ...
  // if(sparkle_mode == kBASIC){
  //   for(int i = 0; i < NUM_SPARKLES;i++){
  //     immediate_sample += sparkle[i].next();
  //   }
  // }
      ...
      switch (i) {
        ...
        case 4:
          if (button_vals[i]) {  // button 4 pressed 
            debugln("B4 pressed"); // activates sparkle mode
            sparkle_mode = kBASIC;
...
void updateSparkle(){
    uint16_t num_active_notes = humanNoteQueue.size();
    uint16_t my_note_num = irand(0, num_active_notes);
    ActiveMidiNote *myNote = humanNoteQueue.get(my_note_num);

    uint16_t my_midi_note = myNote->note_num + 24;

    if(my_midi_note > 127){
      my_midi_note = 127;
    }

    sparkle[sparkle_sample_num].noteOn(sample_freq_table[my_midi_note], 255);
    sparkle_sample_num = (sparkle_sample_num + 1) % NUM_SPARKLES;
}

...
void updateControl() {
  ...
  if (main_mode == kWAVETABLE) {
    ...
    mod.setFreq((float)((float)meap.aux_mux_vals[0] / 2000.0));
    wave.update();
  }
...
void programChangeHandler(int program_num) {
  ...
  switch (main_mode) {
    case kSAMPLER:
      ...
      break;
    case kWAVETABLE:
      if (program_num >= 0 && program_num < NUM_WAVETABLES) {
        wave.setTable(wavetables_list[program_num]);
      }
      break;
  }
}

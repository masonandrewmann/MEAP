...
mSample<MAX_SAMPLE_LENGTH, AUDIO_RATE> sample[POLYPHONY];  //maybe change the MAX_SAMPLE_LENGTH? calc how it corresponds to seconds etc
...
public:
  ...
  DeltaCounter()
    : delta_time(1000), last_time(0), init(false){};
...
public:
  ...
  uint8_t velocity;
  uint8_t voice_num;
  int32_t last_note_id;
  uint8_t num_notes_active;
  uint32_t note_on_count;
  int8_t note_active[127];
  uint16_t note_id[127];
  bool voice_active[127];
  int max_voices;
  bool delta_init;
  float tempo_restrict;

  Borax(uint8_t _max_voices)
    : delta_time(1000), delta_start_time(0), duration_time(1000), duration_start_time(0), num_notes_active(0), note_on_count(0), max_voices(127), delta_init(false), tempo_restrict(0.15)
  {
  ...
  void reset() {
    ...
    delta_start_time = millis();
    duration_start_time = millis();
    delta_time = 1000;
    duration_time = 1000;
    num_notes_active = 0;
    note_on_count = 0;
  ...
  void noteOn(uint8_t _pitch, uint8_t _velocity) {
    if (note_active[_pitch] == -1) {  // is the note currently inactive?
      ...
      for (uint8_t i = 0; i < max_voices; i++) {
        if (!voice_active[i]) {
          note_on_count++;
          voice_active[i] = true;
          note_active[_pitch] = i;
          note_id[_pitch] = note_on_count;
          last_note_id = note_id[_pitch];
          voice_num = i;
          pitch = _pitch;
          velocity = _velocity;
          ...
          duration_start_time = curr_time;
          //start delta timer
          if (delta_init) {
            uint32_t tmp_delta_time = curr_time - delta_start_time;
            if (tmp_delta_time > 40 && tmp_delta_time < 3000) {
              if ((float)tmp_delta_time > ((float)delta_time * (1 - tempo_restrict)) && (float)tmp_delta_time < ((float)delta_time * (1 + tempo_restrict))) {
  ...
  void noteOff(uint8_t _pitch) {
    if (note_active[_pitch] != -1) {
      voice_active[note_active[_pitch]] = false;
      note_active[_pitch] = -1;
      uint32_t curr_time = millis();
      uint32_t tmp_duration_time = curr_time - duration_start_time;
      if (tmp_duration_time > 50) {
        duration_time = tmp_duration_time;
      }
      duration_count++;
    }
  }
...
Borax my_first_borax(POLYPHONY);
...
uint32_t main_delta = 1000;
DeltaCounter my_delta_counter;

// variables for this dumb temporary function
uint32_t last_melody_time = 0;
uint8_t possible_notes[5] = { 36, 38, 40, 43, 45 };
...
void updateControl() {
  ...
  cutoff = (alpha)*cutoff + (1.0 - alpha) * (meap.aux_mux_vals[4] >> 4);
  ...
  resonance = meap.aux_mux_vals[6] >> 4;  // 4095 -> 255
  filter.setResonance(resonance);
  filter2.setResonance(resonance);
...
void noteOnHandler(int note, int velocity, int channel, int pgm_override) {
  if (channel == MIDI_NOTE_CHANNEL) {  // check on which channel we received the MIDI noteOn
    my_first_borax.noteOn(note, velocity);
    last_melody_time = 0;
    // debug("delta: ");
    // debug(my_first_borax.getDelta());
    // debug(" ... duration: ");
    // debugln(my_first_borax.getDuration());
    if (tempo_source == kKeyboard) {
      main_delta = my_first_borax.getDelta();
    }
  ...
  switch (MIDI.getType())  // Get the type of the message we caught
  {
    ...
    case midi::ControlChange:  // ---------- MIDI CONTROL CHANGE RECEIVED ----------
      ...
      switch (data1) {
        ...
        case 20:  // big button
          if (data2 == 127) {
            uint32_t tap_delta = my_delta_counter.next();
            if (tempo_source == kTap) {
              main_delta = tap_delta;
            }
          }
          break;
...
void updateRandomMelody() {
  uint32_t curr_time = millis();
  if (curr_time > (last_melody_time + main_delta)) {
    //   // trigger a note
    computerNoteOn(possible_notes[meap.irand(0, 4)], 127, MIDI_NOTE_CHANNEL, -1, my_first_borax.getDuration());
    //   // reset the timer
    last_melody_time = curr_time;
  }
}
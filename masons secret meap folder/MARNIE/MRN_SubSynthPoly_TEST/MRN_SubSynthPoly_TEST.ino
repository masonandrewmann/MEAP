...
mSubSynthPoly<SAW2048_NUM_CELLS, 3, AUDIO_RATE, CONTROL_RATE, int8_t, 4> poly_voice(SAW2048_DATA);


// mSubSynthVoice<SAW2048_NUM_CELLS, 3, AUDIO_RATE, CONTROL_RATE, int8_t> test_voice(SAW2048_DATA);
...
void setup() {
  ...
  // poly_voice.init();
  poly_voice.setOscGain(0, 255);
  poly_voice.setOscGain(1, 170);
  poly_voice.setOscGain(2, 100);

  poly_voice.setOscSemitones(0, 0);
  poly_voice.setOscSemitones(1, 0);
  poly_voice.setOscSemitones(2, 0);

  // poly_voice.setDetune
}
...
void updateControl() {
  ...
  poly_voice.setCutoff(meap.aux_mux_vals[4]>>4);
  poly_voice.setResonance(meap.aux_mux_vals[3]>>4);

  poly_voice.setAttackTime(meap.aux_mux_vals[6]);
  poly_voice.setReleaseTime(meap.aux_mux_vals[5]);

  

  poly_voice.setFilterEnvAmount(meap.aux_mux_vals[0]>>4);
  poly_voice.setFilterAttackTime(meap.aux_mux_vals[1]>>2);
  poly_voice.setFilterReleaseTime(meap.aux_mux_vals[2]>>2);
  // test_voice.setFilterEnvTimes(meap.aux_mux_vals[1]>>2, 1, 30000, meap.aux_mux_vals[2]>>2);

  // vel = meap.pot_vals[0]>>5;
  // poly_voice.setFilterDecayTime(meap.pot_vals[1]>>2);
  // poly_voice.setFilterSustainLevel(meap.pot_vals[0]>>4);

  poly_voice.setOscDetune(1, ((float)meap.pot_vals[1]) / 4095.0-0.5);
  poly_voice.setOscDetune(2, ((float)meap.pot_vals[0]) / 4095.0-0.5);
...
AudioOutput_t updateAudio() {
  int32_t out_sample = poly_voice.next();
  // int32_t out_sample = 0;
  return StereoOutput::fromNBit(20, out_sample, out_sample);
}

void Meap::updateTouch(int number, bool pressed) {
  static int notes[] = { 48, 50, 52, 53, 55, 57, 59, 60 };
  if (pressed) {
    poly_voice.noteOn(notes[number], vel);
  } else {
    poly_voice.noteOff(notes[number]);
    // test_voice.noteOff();
  }

...
void setup() {
  ...
  uint32_t len = 100;
  float *my_coefficients;
  my_coefficients = (float*)malloc(len * sizeof(float));
  for(uint32_t i = 0; i < len; i++){
    my_coefficients[i] = 0;
  }
  my_coefficients[0] = 1.0;
  my_coefficients[len-1] = 1.0;
  my_first_fir.setCoefficients(my_coefficients, len);
  free(my_coefficients);
}
...
void updateControl() {
  ...
  carrier_osc.setFreq((float)map(meap.pot_vals[0], 0, 4095, 200, 1000));  // set mod oscillator range to 1->1000Hz
}


AudioOutput_t updateAudio() {
  int sample = carrier_osc.next();
  int del_sample = my_first_fir.next(sample);
  // sample = sample + del_sample;
  return StereoOutput::fromAlmostNBit(9, sample, del_sample);  // scale by factor of 4 and apply phase mod to carrier oscillator
}

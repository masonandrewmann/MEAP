...
MEAP_BandedWG<int16_t> my_first_wg;
...
void setup() {
  ...
  my_first_wg.setStrikePosition(0.5);
  my_first_wg.setPreset(2);
  my_first_wg.setFrequency(440.0);
}
...
void updateControl() {
  ...
  my_first_wg.setStrikePosition(((float)meap.pot_vals[0])/4095.f);
}

/** Called automatically at rate specified by AUDIO_RATE macro, for calculating samples sent to DAC, too much code in here can disrupt your output
	*/
AudioOutput_t updateAudio() {
int sample = my_first_wg.next();

  return StereoOutput::fromAlmostNBit(12, sample, sample);
}
...
void Meap::updateTouch(int number, bool pressed) {
  if (pressed) {  // Any pad pressed
    my_first_wg.noteOn(freqs[number], 1.0);
    Serial.println("touched");
  } else {  // Any pad released
  my_first_wg.noteOff(0.5);

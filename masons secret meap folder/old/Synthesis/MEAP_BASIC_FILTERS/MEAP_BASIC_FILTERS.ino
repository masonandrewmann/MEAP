...
MultiResonantFilter<uint8_t> myFilter; // Multifilter applied to an 8-bit signal.
...
void updateControl(){
  ...
  int myCutoff = map(potVals[0], 0, 4095, 0, 255);
  int myResonance = map(potVals[1], 0, 4095, 0, 255);
  myFilter.setCutoffFreqAndResonance(myCutoff, myResonance);
}


int updateAudio(){
  int sig;
  myFilter.next(myWhiteNoise.next());
  switch (filter_type) // recover the output from the current selected filter type.
  {
    case lowpass:
      sig = myFilter.low(); // lowpassed sample
      break;
    case highpass:
      sig = myFilter.high();  // highpassed sample
      break;
    case bandpass:
      sig = myFilter.band();  // bandpassed sample
      break;
    case notch:
      sig = myFilter.notch();  // notched sample
      break;
  }
  return MonoOutput::fromAlmostNBit(10, sig);
}

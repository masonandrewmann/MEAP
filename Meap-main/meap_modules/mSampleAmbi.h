...
#ifndef MEAP_SAMPLEAMBI_H_
#define MEAP_SAMPLEAMBI_H_
...
*/
template <uint64_t NUM_TABLE_CELLS, unsigned int UPDATE_RATE, class T = int8_t, uint8_t INTERP = kINTERP_NONE>
class mSampleAmbi
{
...
public:
    ...
    */
    mSampleAmbi(const T *TABLE_NAME) : table(TABLE_NAME), endpos_fractional((uint64_t)NUM_TABLE_CELLS << SAMPLE_F_BITS) // so isPlaying() will work
    {
        setLoopingOff();
        // rangeWholeSample();
    }
    ...
    */
    mSampleAmbi() : endpos_fractional((uint64_t)NUM_TABLE_CELLS << SAMPLE_F_BITS)
    {
        setLoopingOff();
        // rangeWholeSample();
    }

...
#ifndef MEAP_SAMPLE16_H_
#define MEAP_SAMPLE16_H_
...
@section int16_t2mozzi
Converting soundfiles for Mozzi.
There is a python script called int16_t2mozzi.py in the Mozzi/python folder.
The script converts raw sound data saved from a program like Audacity.
Instructions are in the int16_t2mozzi.py file.
*/
template <uint64_t NUM_TABLE_CELLS, uint32_t UPDATE_RATE, uint16_t INTERP = kINTERP_NONE>
class mSample16
{
...
public:
    ...
    Mozzi by the int16_t2mozzi.py python script in Mozzi's python
    folder.  Sound tables can be of arbitrary lengths for Sample().
    */
    mSample16(const int16_t *TABLE_NAME) : table(TABLE_NAME), endpos_fractional((uint64_t)NUM_TABLE_CELLS << SAMPLE_F_BITS) // so isPlaying() will work
    {
        setLoopingOff();
        // rangeWholeSample();
    }
    ...
    */
    mSample16() : endpos_fractional((uint64_t)NUM_TABLE_CELLS << SAMPLE_F_BITS)
    {
        setLoopingOff();
        // rangeWholeSample();
    }
    ...
    */
    ...
    { // 4us
        ...
        if (INTERP == kINTERP_LINEAR)
        {
            ...
            int16_t diff_fraction = (int16_t)(((((uint64_t)phase_fractional) >> 8) * difference) >> 8); // (unsigned int) phase_fractional keeps low word, then>> for only 8 bit precision
            out += diff_fraction;
        }
        else
        {
    ...
    */
    ...
    //
    // inline
    // int16_t phMod(long phmod_proportion)
    // {
    // 	incrementPhase();
    // 	return FLASH_OR_RAM_READ<const int16_t>(table + (((phase_fractional+(phmod_proportion * NUM_TABLE_CELLS))>>SAMPLE_SAMPLE_F_BITS) & (NUM_TABLE_CELLS - 1)));
    // }
    ...
    */
    inline void setFreq(int32_t frequency)
    {
    ...
    */
    inline int16_t atIndex(uint64_t index)
    {
    ...
    */
    inline uint64_t phaseIncFromFreq(uint32_t frequency)
    {

...
#ifndef MEAP_SAMPLE_H_
#define MEAP_SAMPLE_H_
...
public:
    ...
    */
    mSample(const int8_t *TABLE_NAME) : table(TABLE_NAME), endpos_fractional((uint64_t)NUM_TABLE_CELLS << SAMPLE_F_BITS) // so isPlaying() will work
    {
        setLoopingOff();
        // rangeWholeSample();
    }
    ...
    */
    inline void setTable(const int8_t *TABLE_NAME)
    {
        table = TABLE_NAME;
    }
    ...
    */
    inline int8_t atIndex(uint64_t index)
    {

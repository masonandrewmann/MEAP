...
if len(sys.argv) != 5:
        print ('Usage: char2mozzi16.py <infile outfile tablename samplerate>')
        sys.exit(1)

[infile, outfile, tablename, samplerate] = sys.argv[1:]

def char2mozzi16(infile, outfile, tablename, samplerate):
	fin = open(os.path.expanduser(infile), "rb")
	print ("opened " + infile)
	uint16_tstoread = os.path.getsize(os.path.expanduser(infile))//2
	##print uint8_tstoread
	valuesfromfile = array.array('h') # array of signed int16_t ints
	try:
		valuesfromfile.fromfile(fin, uint16_tstoread)
	finally:
		fin.close()
	...
	fout.write('#define ' + tablename + '_FRAME_SIZE 2048' +'\n')
	fout.write('#define ' + tablename + '_NUM_FRAMES 256' + '\n \n')
...
char2mozzi16(infile, outfile, tablename, samplerate)
